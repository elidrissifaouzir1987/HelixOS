//! Coordinator-owned bounded dispatch and reserved control queue facade.
...
pub const COORDINATOR_DISPATCH_ORDINARY_CAPACITY_V1: usize = 1024;
pub const COORDINATOR_DISPATCH_CONTROL_CAPACITY_V1: usize = 32;
pub const COORDINATOR_DISPATCH_BACKPRESSURE_LIMIT_MS_V1: u64 = 50;
pub const COORDINATOR_DISPATCH_CONTROL_P99_LIMIT_MS_V1: u64 = 100;
pub const COORDINATOR_DISPATCH_CONTROLLED_TRIALS_V1: usize = 100;
pub const COORDINATOR_DISPATCH_DUPLICATE_FLOOD_V1: usize = 10_000;

const _: [(); COORDINATOR_DISPATCH_ORDINARY_CAPACITY_V1] =
    [(); DISPATCH_QUEUE_ORDINARY_CAPACITY_V1];
const _: [(); COORDINATOR_DISPATCH_CONTROL_CAPACITY_V1] = [(); DISPATCH_QUEUE_CONTROL_CAPACITY_V1];
const _: [(); COORDINATOR_DISPATCH_CONTROLLED_TRIALS_V1] =
    [(); DISPATCH_QUEUE_CONTROLLED_TRIALS_V1];
const _: [(); COORDINATOR_DISPATCH_DUPLICATE_FLOOD_V1] = [(); DISPATCH_QUEUE_DUPLICATE_FLOOD_V1];
const _: [(); COORDINATOR_DISPATCH_BACKPRESSURE_LIMIT_MS_V1 as usize] =
    [(); DISPATCH_QUEUE_ORDINARY_BACKPRESSURE_LIMIT_MS_V1 as usize];
const _: [(); COORDINATOR_DISPATCH_CONTROL_P99_LIMIT_MS_V1 as usize] =
    [(); DISPATCH_QUEUE_CONTROL_P99_LIMIT_MS_V1 as usize];

/// Coordinator-facing payload-free metric publication.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct CoordinatorDispatchQueueMetricsSnapshotV1 {
...
impl From<DispatchQueueMetricsSnapshotV1> for CoordinatorDispatchQueueMetricsSnapshotV1 {
    fn from(snapshot: DispatchQueueMetricsSnapshotV1) -> Self {
        Self {
            ...
            dispatch_count: snapshot.dispatch_count_v1(),
...
/// In-memory queue accounting owned by the coordinator process.
///
/// Durable dispatch state remains in its sovereign store. This facade retains only
/// opaque pending bindings and bounded counters, so it cannot become another store.
pub struct CoordinatorDispatchQueueV1 {
    queue: DispatchQueueV1,
}

impl CoordinatorDispatchQueueV1 {
    pub fn new_v1() -> Self {
        Self {
    ...
    pub fn admit_dispatch_v1(&mut self, binding: [u8; 32]) -> DispatchQueueAdmissionV1 {
        let admission = self
            .queue
            .admit_ordinary_v1(DispatchQueueBindingV1::new_v1(binding));
        if admission.accepted_v1() {
            self.queue.observe_dispatch_v1();
        }
        admission
    }
    ...
    pub fn dequeue_dispatch_v1(&mut self) -> Option<[u8; 32]> {
        self.queue
            .dequeue_ordinary_v1()
    ...
    pub fn metrics_snapshot_v1(&self) -> CoordinatorDispatchQueueMetricsSnapshotV1 {
...
impl Default for CoordinatorDispatchQueueV1 {
    fn default() -> Self {
        Self::new_v1()
    }
}

impl fmt::Debug for CoordinatorDispatchQueueV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CoordinatorDispatchQueueV1")
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CoordinatorDispatchQueueProfileErrorV1 {
...
/// Measures the production queue facade over exactly 100 saturation trials.
///
/// Every trial fills all 1024 ordinary slots, submits 10,000 exact duplicates, measures
/// one immediate ordinary backpressure decision, then measures PAUSE, status and
/// reconciliation through the independent capacity-32 control lane. Threshold verdicts
/// are carried by the returned nearest-rank p99 profile rather than hidden assertions.
pub fn measure_coordinator_dispatch_queue_profile_v1(
) -> Result<DispatchQueueControlledProfileV1, CoordinatorDispatchQueueProfileErrorV1> {
    let mut trials =
        [DispatchQueueTrialMeasurementV1::ZERO; COORDINATOR_DISPATCH_CONTROLLED_TRIALS_V1];

    for (trial_index, trial) in trials.iter_mut().enumerate() {
        let mut queue = CoordinatorDispatchQueueV1::new_v1();
        for ordinal in 0..COORDINATOR_DISPATCH_ORDINARY_CAPACITY_V1 {
            if queue.admit_dispatch_v1(profile_binding_v1(1, trial_index, ordinal))
                != DispatchQueueAdmissionV1::Accepted
            {
                return Err(CoordinatorDispatchQueueProfileErrorV1::OrdinaryFillInvariant);
            }
        }

        let retained = profile_binding_v1(1, trial_index, 0);
        for _ in 0..COORDINATOR_DISPATCH_DUPLICATE_FLOOD_V1 {
            if queue.admit_dispatch_v1(retained) != DispatchQueueAdmissionV1::ExactDuplicate {
                return Err(CoordinatorDispatchQueueProfileErrorV1::DuplicateInvariant);
            }
        }

        let ordinary_started = Instant::now();
        let ordinary_admission = queue.admit_dispatch_v1(profile_binding_v1(
            1,
            trial_index,
            COORDINATOR_DISPATCH_ORDINARY_CAPACITY_V1,
        ));
        let ordinary_backpressure_ms = elapsed_ms_v1(ordinary_started);
        if ordinary_admission
            != (DispatchQueueAdmissionV1::Backpressured {
                within_ms: COORDINATOR_DISPATCH_BACKPRESSURE_LIMIT_MS_V1,
            })
        {
            return Err(CoordinatorDispatchQueueProfileErrorV1::OrdinaryBackpressureInvariant);
        }
...
) -> Result<(), CoordinatorDispatchQueueProfileErrorV1> {
    if admission == DispatchQueueAdmissionV1::Accepted {
        Ok(())
    } else {
        Err(CoordinatorDispatchQueueProfileErrorV1::ControlAdmissionInvariant)
    }
}
...
mod tests {
    ...
    fn ordinary_and_control_lanes_remain_independent() {
        ...
        for ordinal in 0..COORDINATOR_DISPATCH_ORDINARY_CAPACITY_V1 {
            assert_eq!(
                queue.admit_dispatch_v1(profile_binding_v1(1, 0, ordinal)),
                DispatchQueueAdmissionV1::Accepted
            );
        }
        assert_eq!(
            queue.admit_dispatch_v1(profile_binding_v1(1, 0, 2_000)),
            DispatchQueueAdmissionV1::Backpressured {
                within_ms: COORDINATOR_DISPATCH_BACKPRESSURE_LIMIT_MS_V1
            }
        );
        assert_eq!(
            queue.admit_pause_v1(profile_binding_v1(2, 0, 0)),
            DispatchQueueAdmissionV1::Accepted
        );
